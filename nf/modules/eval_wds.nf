process EVAL_WDS {
  ...
  input:
    // global-calibration aware: 4-tuple
    tuple val(sample_id), path(wds_dir), path(ckpt), path(calib_json)
    path eval_py
    path model_py
    val  dataset_id
    val  exp_name
  ...
  echo "[EVAL] calib_json=${calib_json}"
  ...
  # Build *local path* shard lists (status quo: wds_dir is staged locally)
  ...
  #  - If calib_json exists AND is not a sentinel (NO_CALIB), apply-only:
  #      --calib_params_json calib_json
  #  - Else fallback to per-eval fit+apply logic (existing)
  # ---------------------------
  APPLY_GLOBAL_ARGS=""
  CALIB_ARGS=""
  CALIB_FIT_ARGS=""
  ...
  if [ -f "${calib_json}" ] && [ "\$(basename "${calib_json}")" != "NO_CALIB" ]; then
    echo "[EVAL] Using GLOBAL calibrator: ${calib_json}"
    APPLY_GLOBAL_ARGS="--calib_params_json ${calib_json}"
  else
    echo "[EVAL] No global calibrator; using per-eval calibration settings (if enabled)."

    # Decide per-eval calibration args (fit+apply)
    ...
    # Optional: clamp workers for calib-fit split too
